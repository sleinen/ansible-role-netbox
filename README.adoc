...
Toggle `netbox_ldap_debug` to `true` if you want LDAP debugging output
on the console.  Note that this may disable other logging output, so
please use this only when specifically debugging LDAP authentication.
[source,yaml]
----
